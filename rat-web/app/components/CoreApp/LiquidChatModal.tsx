...
export function LiquidChatModal(props: LiquidChatModalProps) {
  ...
  const STORAGE_KEY = 'liquid-chat-instances';
  const ACTIVE_ID_KEY = 'liquid-chat-active-id';
  ...
  const loadSavedData = () => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved) as ChatInstance[];
        // Reset status to idle when loading
        return {
          instances: parsed.map(inst => ({ ...inst, status: 'idle' as const })),
          activeId: savedActiveId || parsed[0]?.id || '1'
        };
      }
    } catch (e) {
    ...
    return {
      instances: [{ id: '1', content: '', status: 'idle', messages: [] }],
      activeId: '1'
    };
  };
  
  const savedData = loadSavedData();
  const [chatInstances, setChatInstances] = createSignal<ChatInstance[]>(savedData.instances);
  const [activeInstanceId, setActiveInstanceId] = createSignal(savedData.activeId);
  const [swipeStartX, setSwipeStartX] = createSignal(0);
  const [swipeX, setSwipeX] = createSignal(0);
  const [isAnimating, setIsAnimating] = createSignal(false);
  
  // Save to localStorage whenever instances or active ID changes
  createEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(chatInstances()));
      localStorage.setItem(ACTIVE_ID_KEY, activeInstanceId());
    } catch (e) {
  ...
  onMount(() => {
    if (modalRef) {
      modalRef.addEventListener('touchstart', handleTouchStart, { passive: true });
      modalRef.addEventListener('touchmove', handleTouchMove, { passive: true });
      modalRef.addEventListener('touchend', handleTouchEnd, { passive: true });
    }
          ...
          >
            ...
            <div class="h-full bg-gradient-to-t from-black/90 via-black/80 to-black/60 backdrop-blur-2xl border-t border-white/10 rounded-t-3xl shadow-2xl">
              ...
              <div class="flex items-center justify-center gap-3 px-4 pb-3">
                {/* Swipe indicator */}
                <Show when={Math.abs(swipeX()) > 20}>
                  <div class="absolute left-4 text-white/40 text-xs animate-pulse">
                    {swipeX() > 0 && chatInstances().findIndex(i => i.id === activeInstanceId()) > 0 && '←'}
                  </div>
                  <div class="absolute right-4 text-white/40 text-xs animate-pulse">
                    {swipeX() < 0 && chatInstances().findIndex(i => i.id === activeInstanceId()) < chatInstances().length - 1 && '→'}
                  </div>
                </Show>
                  ...
                  {(instance) => (
                    <button
                      ...
                      class={`relative w-3 h-3 rounded-full transition-all duration-300 ${getDotColor(instance)} ${
                        instance.id === activeInstanceId() ? 'scale-125' : 'scale-100 opacity-60 hover:opacity-80'
                      }`}
              ...
              <div class={`h-[calc(100%-5rem)] px-4 pb-4 rounded-2xl mx-3 transition-colors duration-500 ${
                ...
                <div class="h-full flex flex-col">
                  ...
                  <div class="flex items-center justify-between mt-2 text-xs text-white/40">
                    <span>⌘+Enter to send • {chatInstances().length > 1 ? 'Swipe or Ctrl+Tab to switch' : 'Tap + to add chat'}</span>
                    <Show when={chatInstances().length > 1}>
                      <button
